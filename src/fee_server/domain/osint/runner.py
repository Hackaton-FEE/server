...
def _run_engine(
    ...
    try:
        ...
        _record_engine_result(engine_state, engine.name, result.status, started, count)
        ...
        _record_engine_result(engine_state, engine.name, ENGINE_ERROR, started, 0, error=True)
        return []
...
def _record_engine_result(
    ...
    *,
    ...
    if error:
        entry["error_category"] = "engine-exception"
